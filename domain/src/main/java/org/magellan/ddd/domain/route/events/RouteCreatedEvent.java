...
public record RouteCreatedEvent(RouteId routeId, RouteStatus status, UserId dispatcherId, Address address) {

  public static RouteCreatedEvent of(CreateRouteCommand command) {
    return new RouteCreatedEvent(command.routeId(), RouteStatus.NEW, command.dispatcherId(), command.address());
  }
